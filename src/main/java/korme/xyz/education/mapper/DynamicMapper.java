...
public interface DynamicMapper {
     ...
     * userType:园长
     * */
    @Select("select d.userId,d.dynamicId,d.images,d.date,d.excerpt,u.nickName,u.headPortrait,u.userType, d.commentNum,d.pointNum," +
            "d.browseNum,IFNULL(p.pointId,0) pointId  from dynamic as d LEFT JOIN user as u ON d.userId=u.userId left join (select * from point where userId=#{userId} and pointType=3) p on p.pointedId=dynamicId  where " +
    ...
    List<Map<String,Object>> selectPrincipalAll(@Param("userId") Integer userId,@Param("kidgardenId")Integer kidgardenId);
    ...
    List<Map<String,Object>> selectPrincipalBeforeTime(@Param("userId") Integer userId,@Param("kidgardenId")Integer kidgardenId,@Param("dynamicId")Integer dynamicId);

    /*
     * 返回官方发送的所有动态，时间倒序
     * userType:官方
     * */
    @Select("select d.userId,d.dynamicId,d.images,d.date,d.transDynamicId,d.excerpt,u.nickName,u.headPortrait,u.userType, d.commentNum,d.pointNum," +
            "d.browseNum,IFNULL(p.pointId,0) pointId  from dynamic as d LEFT JOIN user as u ON d.userId=u.userId left join (select * from point where userId=#{userId} and pointType=3) p on p.pointedId=dynamicId  where " +
            "u.userType=4 and d.delState=0 and u.delState=0 order by d.dynamicId DESC")
    List<OfficialDynamicModel> selectOfficialAll(@Param("userId") Integer userId);
    ...
    List<OfficialDynamicModel> selectOfficialBeforeTime(@Param("userId") Integer userId,@Param("dynamicId")Integer dynamicId);
    ...
    List<Map<String,Object>> selectAllPrincipalAll(@Param("userId") Integer userId);
    ...
    List<Map<String,Object>> selectAllPrincipalBeforeTime(@Param("userId") Integer userId,@Param("dynamicId")int dynamicId);

    /*
     * 根据Id选择某条动态
     * */
    @Select("select d.userId,d.dynamicId,d.images,d.date,d.excerpt,u.nickName,u.headPortrait,u.userType,u.userId, d.commentNum,d.pointNum, d.browseNum,d.content,IFNULL(p.pointId,0) pointId  from dynamic as d LEFT JOIN user as u ON d.userId=u.userId  left join (select * from point where pointedId=#{dynamicId} and userId=#{userId} and pointType=3) p on p.pointedId=dynamicId  where d.dynamicId=#{dynamicId} and d.delState=0")
    DynamicOrdinaryModel selectDynamicById(@Param("userId") Integer userId,@Param("dynamicId")int dynamicId);

    /*
     * 根据Id选择某条Offical动态
     * */
    @Select("select d.userId,d.dynamicId,d.images,d.transDynamicId,d.date,d.excerpt,u.nickName,u.headPortrait,u.userType,u.userId, d.commentNum,d.pointNum, d.browseNum,d.content,IFNULL(p.pointId,0) pointId  from dynamic as d LEFT JOIN user as u ON d.userId=u.userId left join (select * from point where pointedId=#{dynamicId} and  userId=#{userId} and pointType=3) p on p.pointedId=dynamicId  where d.dynamicId=#{dynamicId} and d.delState=0 ")
    OfficialDynamicModel selectOffDynamicById(@Param("userId") Integer userId,@Param("dynamicId")int dynamicId);
    ...
    * */
    ...
    class DynamicProvider{
        ...
        public String findClassDynamicUp(Integer userId,Integer userType,Integer kidgardenId,Integer classId){
            SQL sql=new SQL();
            sql.SELECT("d.userId,d.dynamicId,d.images,d.date,d.excerpt,u.nickName," +
                    "u.headPortrait,u.userType, d.commentNum,d.pointNum,d.browseNum,IFNULL(p.pointId,0) pointId ");
            sql.FROM("dynamic as d LEFT JOIN user as u ON d.userId=u.userId left join (select * from point where userId="+userId+" and pointType=3) p on p.pointedId=dynamicId ");
            ...
            if(userType==teacher||userType==parent)
                ...
                sql.WHERE(" d.userId!="+userId+" and d.delState=0 and u.delState=0");

            sql.ORDER_BY("d.dynamicId");
            return sql.toString()+" DESC";
        }

        public String findClassDynamicDown(Integer userId,Integer userType,Integer kidgardenId,Integer classId,int dynamicId){
            SQL sql=new SQL();
            sql.SELECT("d.userId,d.dynamicId,d.images,d.date,d.excerpt,u.nickName," +
                    "u.headPortrait,u.userType, d.commentNum,d.pointNum,d.browseNum,IFNULL(p.pointId,0) pointId ");
            sql.FROM("dynamic as d LEFT JOIN user as u ON d.userId=u.userId left join (select * from point where userId="+userId+" and pointType=3) p on p.pointedId=dynamicId ");
            ...
            if(userType==teacher||userType==parent)
                ...
                sql.WHERE(" d.userId!="+userId+" and  d.dynamicId<'"+dynamicId+"' and d.delState=0 and u.delState=0");
            sql.ORDER_BY("d.dynamicId");
            return sql.toString()+" DESC";
        }
